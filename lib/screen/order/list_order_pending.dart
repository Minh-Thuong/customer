...
class ListOrderPending extends StatefulWidget {
  const ListOrderPending({super.key});

  @override
  State<ListOrderPending> createState() => _ListOrderPendingState();
}

class _ListOrderPendingState extends State<ListOrderPending> {
  ...
  void _navigateToDetail(Order order) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => OrderDetailScreen2(
          order: order,
          onClose: () {
            // Callback này sẽ được gọi từ màn hình chi tiết
            context
                .read<CartBloc>()
                .add(LoadOrderByStatusEvent(status: OrderStatus.PENDING));
          },
        ),
      ),
    );
